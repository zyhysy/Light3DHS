...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        results. # 一个通用的神经网络训练类,适用于除RNN外几乎所有的模型。
        ...
        if deterministic: # 使训练具有确定性
            ...
            torch.backends.cudnn.benchmark = True #设置为True，将会让程序在开始时花费一点额外时间，为整个网络的每个卷积层搜索最适合它的卷积实现算法，进而实现网络的加速。
        ...
        self.dataset = None  # these can be None for inference mode:对于推理模式，这些可以是无
        ...
        self.train_loss_MA = None  # 如果有多个任务监督，这里主要记录主任务
    ...
    def do_split(self):
        ...
        if not isfile(splits_file):
            ...
            splits = []
            all_keys_sorted = np.sort(list(self.dataset.keys())) #self.dataset = load_dataset(self.folder_with_preprocessed_data) #"nnUNet_preprocessed/Task025_Harp/nnUNetData_plans_v2.1_stage0"
    ...
    def _maybe_init_amp(self):
        if self.fp16 and self.amp_grad_scaler is None:
            self.amp_grad_scaler = GradScaler() #GradScaler用于动态图模式下的"自动混合精度"的训练。它控制loss的缩放比例,有助于避免浮点数溢出的问题
    ...
    def run_training(self):
        ...
        maybe_mkdir_p(self.output_folder)        
        self.plot_network_architecture() #绘制网络架构

        if cudnn.benchmark and cudnn.deterministic:#是否需要确定性训练
        ...
        while self.epoch < self.max_num_epochs:
            ...
            # train one epoch
            self.network.train() #说明现在是训练

            if self.use_progress_bar:# 如果使用tqdm显示训练进度
                with trange(self.num_batches_per_epoch) as tbar: 
                    for b in tbar:
                        tbar.set_description("Epoch {}/{}".format(self.epoch+1, self.max_num_epochs))

                        l = self.run_iteration(self.tr_gen, True) # 得到训练损失

                        tbar.set_postfix(loss=l)
                        train_losses_epoch.append(l) #保存损失
            else:
                for _ in range(self.num_batches_per_epoch): # 每个batch进行迭代
                    l = self.run_iteration(self.tr_gen, True)
                    train_losses_epoch.append(l)

            self.all_tr_losses.append(np.mean(train_losses_epoch))#保存一个num_batches_per_epoch平均损失
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gen, False, True) #第二个参数为false则 不进行梯度反传
                    val_losses.append(l)
    ...
    def manage_patience(self):  #早停法
        ...
        if self.patience is not None:
            ...
            # increase patience ：现在看训练损失的移动平均值是否有所改善。如果是，则重置patience，否则继续训练
            if self.train_loss_MA + self.train_loss_MA_eps < self.best_MA_tr_loss_for_patience:
                self.best_MA_tr_loss_for_patience = self.train_loss_MA
                self.best_epoch_based_on_MA_tr_loss = self.epoch
                self.print_to_log_file("New best epoch (train loss MA): %03.4f" % self.best_MA_tr_loss_for_patience)
                self.print_to_log_file("New best epoch : %d" % self.best_epoch_based_on_MA_tr_loss)
            else:
                pass
    ...
    def on_epoch_end(self):
        ...
        self.plot_progress() # 画出训练的progress.png

        self.maybe_update_lr() # 更新学习率

        self.maybe_save_checkpoint() # 保存当前的模型参数、学习率设置、优化器参数等
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            with autocast():  # 上下文管理器，允许脚本区域以混合精度训练
                output = self.network(data)
                del data  # 减少内存
                l = self.loss(output, target)

            if do_backprop: # 如果反省传播
                self.amp_grad_scaler.scale(l).backward() # 在特定位置求梯度值
                self.amp_grad_scaler.step(self.optimizer) # 进行梯度下降,反向传播，更新模型参数
