...
from training.data_augmentation.default_data_augmentation import default_3D_augmentation_params, get_default_augmentation, get_patch_size
from training.dataloading.dataset_loading import load_dataset, DataLoader3D, unpack_dataset
...
#通用的UNet训练类，主要根据plan.pkl文件生成训练信息，还有一些默认参数。
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        :param plans_file: the pkl file generated by preprocessing. This file will determine all design choices #预处理生成的pkl文件。此文件将确定所有设计选择
        ...
        Can be None if you are doing inference only.#预处理数据存在的位置，将被用来训练。如果只进行推理，可以为None。
        ...
        required and may be set to None #存储预处理任务数据的父目录。测试时可以为None
        ...
        is considerably slower! Running unpack_data=False with 2d should never be done!#为False，npz预处理的数据将不会解压到npy。这会占用更少的空间，但
速度相当慢！
    ...
    def initialize_network(self):
        ...
        self.network = Light3DHS(self.num_input_channels, self.base_num_features, self.num_classes, net_numpool,
    ...
    # 绘制网络架构
        try:
            from utilities.file_and_folder_operations import join
    ...
    def process_plans(self, plans):
        if self.stage is None:
            ...
            self.stage = list(plans['plans_per_stage'].keys())[0] # 0
        self.plans = plans # 预处理pkl文件中的内容

        """
        stage_plans:
        {'batch_size': 10, 'num_pool_per_axis': [4, 4, 4], 'patch_size': array([64, 64, 64]), 'median_patient_size_in_voxels': array([64, 64, 64]), 
        'current_spacing': array([1., 1., 1.]), 'original_spacing': array([1., 1., 1.]), 'do_dummy_2D_data_aug': False, 
        'pool_op_kernel_sizes': [[2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2]], 'conv_kernel_sizes': [[3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3]]}
        """
        stage_plans = self.plans['plans_per_stage'][self.stage]
        self.batch_size = stage_plans['batch_size'] # stage_plans['batch_size']:10
        self.net_pool_per_axis = stage_plans['num_pool_per_axis'] #[4,4,4]
        self.patch_size = np.array(stage_plans['patch_size']).astype(int) #[64, 64, 64]
        self.do_dummy_2D_aug = stage_plans['do_dummy_2D_data_aug'] # False
        ...
        if 'pool_op_kernel_sizes' not in stage_plans.keys():
            ...
            #self.net_num_pool_op_kernel_sizes = stage_plans['pool_op_kernel_sizes'] # [[2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2]]
        ...
        if 'conv_kernel_sizes' not in stage_plans.keys():
            ...
            # self.net_conv_kernel_sizes = stage_plans['conv_kernel_sizes'] # [[3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3]]
            self.net_conv_kernel_sizes = stage_plans['conv_kernel_sizes']

        self.pad_all_sides = None  # self.patch_size
        self.intensity_properties = plans['dataset_properties']['intensityproperties'] # None
        self.normalization_schemes = plans['normalization_schemes'] #OrderedDict([(0, 'nonCT')])
        self.base_num_features = 16  # plans['base_num_features']
        self.num_input_channels = plans['num_modalities'] # 1
        self.num_classes = plans['num_classes'] + 1  # background is no longer in num_classes
        self.classes = plans['all_classes'] # [1]
        self.use_mask_for_norm = plans['use_mask_for_norm'] # OrderedDict([(0, False)])
        self.only_keep_largest_connected_component = plans['keep_only_largest_region']  # None
        self.min_region_size_per_class = plans['min_region_size_per_class'] # None
        ...
        self.transpose_forward = plans['transpose_forward'] # [0, 1, 2]
        self.transpose_backward = plans['transpose_backward'] # [0, 1, 2]
        ...
        if "conv_per_stage" in plans.keys():  # this ha sbeen added to the plans only recently
            self.conv_per_stage = plans['conv_per_stage'] # 2
        else:
            self.conv_per_stage = 2

    def load_dataset(self):
        self.dataset = load_dataset(self.folder_with_preprocessed_data) #"nnUNet_preprocessed/Task025_Harp/nnUNetData_plans_v2.1_stage0"

    def get_basic_generators(self):
        self.load_dataset() #加载数据集
        self.do_split() #使用五折交叉验证划分数据集，nnUNetTrainerV2重写了该函数
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        如果debug=True, 则将保留为后处理确定生成的临时文件
        """
        ...
        if self.dataset_val is None:  # self.dataset_val为验证集列表
            self.load_dataset()
            self.do_split()
        ...
        output_folder = join(self.output_folder, validation_folder_name) #nnUNet_trained_models/nnUNet/3d_fullres/Task025_Harp/nnUNetTrainerV2__nnUNetPlansv2.1/fold_0/validation_raw
        ...
        for k in self.dataset_val.keys():
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii"))) or \
                ...
                print(k, data.shape)  # eg: 002_S_0295_001_L (2, 64, 64, 64)
                data[-1][data[-1] == -1] = 0
