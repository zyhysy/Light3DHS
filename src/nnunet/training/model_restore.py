...
def recursive_find_python_class(folder, trainer_name, current_module):
    tr = None
    for importer, modname, ispkg in pkgutil.iter_modules(folder): #pkgutil获取包里面的所有模块列表,ef:common_utils False、DatasetAnalyzer False
        # print(modname, ispkg)
        if not ispkg:
            m = importlib.import_module(current_module + "." + modname)
            if hasattr(m, trainer_name): #判断对象是否包含对应的属性,有该属性返回 True，否则返回 False
                tr = getattr(m, trainer_name) #返回一个对象属性值
                break
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    search_in = join("/home/amax/repository/zyh/My_nnUnet/nnunet", "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="training.network_training")  #network_training.nnUNetTrainerV2.nnUNetTrainerV2
    ...
    trainer.process_plans(info['plans']) ##通过次函数加载plans_file，给一些参数重新赋值。
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    trainer = restore_model(join(folds[0], "%s.model.pkl" % checkpoint_name), fp16=mixed_precision) #得到以及加载过训练参数的模型
