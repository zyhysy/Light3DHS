...
def verify_all_same_orientation(folder):
    ...
    :return:
    """
    nii_files = subfiles(folder, suffix=".nii", join=True)
    orientations = []
    for n in nii_files:
        img = nib.load(n)
        affine = img.affine
...
# label与image应该具有相同的原点、分辨率、方向和大小
...
def verify_dataset_integrity(folder):
    ...
    expected_test_identifiers = [i['image'].split("/")[-1][:-4] for i in test_cases]

    ## check training set:得到所有的训练数据和label
    nii_files_in_imagesTr = subfiles((join(folder, "imagesTr")), suffix=".nii", join=False)
    nii_files_in_labelsTr = subfiles((join(folder, "labelsTr")), suffix=".nii", join=False)
    ...
    for c in expected_train_identifiers:
        ...
        expected_label_file = join(folder, "labelsTr", c + ".nii")
        label_files.append(expected_label_file)
        expected_image_files = [join(folder, "imagesTr", c + "_%04.0d.nii" % i) for i in range(num_modalities)]
    ...
    # verify that only properly declared values are present in the labels:判断label中的值是否都在json中
    print("Verifying label values")
    expected_labels = list(int(i) for i in dataset['labels'].keys())
    expected_labels.sort()

    # check if labels are in consecutive order:判断标签是否按连续顺序排列
    ...
    if len(expected_test_identifiers) > 0:
        ...
        nii_files_in_imagesTs = subfiles((join(folder, "imagesTs")), suffix=".nii", join=False)

        for c in expected_test_identifiers:
            # check if all files are present
            expected_image_files = [join(folder, "imagesTs", c + "_%04.0d.nii" % i) for i in range(num_modalities)]
            assert all([isfile(i) for i in
                        expected_image_files]), "some image files are missing for case %s. Expected files:\n %s" % (
                c, expected_image_files)
