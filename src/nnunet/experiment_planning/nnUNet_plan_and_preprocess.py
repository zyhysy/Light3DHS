...
"""
 no_pp 设置为true, 则不会进行预处理,tl、tf 设置预处理时所用的进程，
 verify_dataset_integrity 用于设置是是否检查数据完整性,如果设置为True,则将对数据完整性进行检查。   
"""
...
def main():
    ...
    parser.add_argument("-tf", type=int, required=False, default=8,
                        help="Number of processes used for preprocessing the full resolution data of the 2D U-Net and "
                             "3D U-Net. Don't overdo it or you will run out of RAM") #预处理2D U-net和3D U-net的全分辨率数据的进程数
    ...
    task_ids = args.task_ids   # 25
    dont_run_preprocessing = args.no_pp  # False
    tl = args.tl
    tf = args.tf 
    ...
    for i in task_ids:
        ...
        task_name = convert_id_to_task_name(i) # Task025_Harp
        
        args.verify_dataset_integrity = True
        if args.verify_dataset_integrity:
            verify_dataset_integrity(join(nnUNet_raw_data, task_name))

        crop(task_name, False, tf) # 对数据去黑边，并将data、seg保存为npz文件，properties保存为pkl文件

        tasks.append(task_name)

    search_in = join('/home/amax/repository/zyh/My_nnUnet/nnunet',  "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="experiment_planning") # 从experiment_planning包下某个.py文件中找到ExperimentPlanner3D_v21类
    ...
    for t in tasks:
        print("\n\n\n", t)
        cropped_out_dir = os.path.join(nnUNet_cropped_data, t)  # My_nnUnet/dataset/nnUNet_raw/nnUNet_cropped_data/Task025_Harp
        preprocessing_output_dir_this_task = os.path.join(preprocessing_output_dir, t)  # My_nnUnet/dataset/nnUNet_preprocessed/Task025_Harp
        ...
        collect_intensityproperties = True if (("CT" in modalities) or ("ct" in modalities)) else False  # False
        ...
        if planner_3d is not None:
            if args.overwrite_plans is not None:
                ...
                exp_planner = planner_3d(cropped_out_dir, preprocessing_output_dir_this_task)  # 加载nnUNet_plan_and_preprocess.ExperimentPlanner3D_v21
