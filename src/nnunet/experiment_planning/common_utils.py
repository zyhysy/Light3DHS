...
def split_4d_nifti(filename, output_folder):
    ...
    dim = img_itk.GetDimension()  #读取图像维度
    file_base = filename.split("/")[-1]  #获取文件名
    if dim == 3: # 如果是3D数据，直接将数据命名拷过去
        shutil.copy(filename, os.path.join(output_folder, file_base[:-4] + "_0000.nii"))
        return
    elif dim != 4: #既不是3维也不是4维则不支持该模式
        raise RuntimeError("Unexpected dimensionality: %d of file %s, cannot split" % (dim, filename))
    else:
        img_npy = sitk.GetArrayFromImage(img_itk) #将simpleITK类型的对象转换为numpy类型的数组
        spacing = img_itk.GetSpacing()
        origin = img_itk.GetOrigin()
        direction = np.array(img_itk.GetDirection()).reshape(4,4)
        # now modify these to remove the fourth dimension
        spacing = tuple(list(spacing[:-1])) #删除掉spacing、origin、direction第四个维度
        ...
        for i, t in enumerate(range(img_npy.shape[0])): #不同的模态
            ...
            sitk.WriteImage(img_itk_new, os.path.join(output_folder, file_base[:-4] + "_%04.0d.nii" % i))
