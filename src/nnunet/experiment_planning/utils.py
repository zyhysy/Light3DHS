...
import numpy as np
from utilities.file_and_folder_operations import join, isdir, maybe_mkdir_p, subfiles, subdirs, isfile
from configuration import default_num_threads
from DatasetAnalyzer import DatasetAnalyzer
from common_utils import split_4d_nifti
...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    ...
    full_task_name = input_folder.split("/")[-1] #获取完整的task name:Task24_HarpL
    ...
    task_name = full_task_name[7:] # HarpL
    ...
    for subdir in ["imagesTr", "imagesTs"]:
        ...
        nii_files = [join(curr_dir, i) for i in os.listdir(curr_dir) if i.endswith(".nii") or i.endswith(".nii.gz")] ##获取所有nifti文件并组合成完整路径
        nii_files.sort()
        for n in nii_files:
            files.append(n)
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    from experiment_planner_baseline_3DUNet import ExperimentPlanner
