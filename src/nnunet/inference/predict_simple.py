...
from utilities.file_and_folder_operations import join, isdir
...
def main():
    ...
    parser.add_argument('-f', '--folds', nargs='+', default='None',
                        help="folds to use for prediction. Default is None which means that folds will be detected "
                             "automatically in the model output folder") # 用于预测的fold。默认值为“无”，这意味着将在模型输出文件夹中自动检测fold
    ...
    parser.add_argument('-z', '--save_npz', required=False, action='store_true',
                        help="use this if you want to ensemble these predictions with those of other models. Softmax "
                             ...
                             "merged between output_folders with nnUNet_ensemble_predictions") #如果要将这些预测与其他模型的预测进行集成，请使用此选项。Softmax概率将保存到输出文件夹中
    ...
    parser.add_argument('-l', '--lowres_segmentations', required=False, default='None',
                        help="if model is the highres stage of the cascade then you can use this folder to provide "
                             ...
                             "network weights are present") #如果模型是级联的高分辨率阶段，则可以使用此文件夹从低分辨率3D U-Net提供预测。
    ...
    parser.add_argument("--part_id", type=int, required=False, default=0, help="Used to parallelize the prediction of "
                                                                               ...
                                                                               "GPU (for example via "
                                                                               "CUDA_VISIBLE_DEVICES=X)")#用于在多个GPU上并行预测文件夹。如果要使用n个GPU预测此文件夹，则需要运行此命令
    ...
    "run into out of memory (RAM) problems. Default: 6") #确定多少后台进程将用于数据预处理。
    ...
    parser.add_argument('-chk',
                        help='checkpoint name, default: model_final_checkpoint or model_best',
                        required=False,
    ...
    part_id = args.part_id  # 0
    num_parts = args.num_parts  # 1
    folds = args.folds  # None
    save_npz = args.save_npz  # False
    lowres_segmentations = args.lowres_segmentations  # None
    num_threads_preprocessing = args.num_threads_preprocessing  # 6
    num_threads_nifti_save = args.num_threads_nifti_save  # 2
    disable_tta = args.disable_tta  # False
    step_size = args.step_size  # 0.5
    # interp_order = args.interp_order
    # interp_order_z = args.interp_order_z
    # force_separate_z = args.force_separate_z
    overwrite_existing = args.overwrite_existing  # False
    mode = args.mode  # normal
    all_in_gpu = args.all_in_gpu  # None
    model = args.model   # 3d_fullres
    trainer_class_name = args.trainer_class_name  # nnUNetTrainerV2
    cascade_trainer_class_name = args.cascade_trainer_class_name  # nnUNetTrainerV2CascadeFullRes

    task_name = args.task_name 

    if not task_name.startswith("Task"):
        task_id = int(task_name)
        task_name = convert_id_to_task_name(task_id) # Task025_Harp
    ...
    model_folder_name = join(network_training_output_dir, model, task_name, trainer + "__" +
                              args.plans_identifier) #dataset/nnUNet_trained_models/nnUNet/3d_fullres/Task025_Harp/nnUNetTrainerV2__nnUNetPlansv2.1
