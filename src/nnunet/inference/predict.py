...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    num_processes = min(len(list_of_lists), num_processes) # 2

    classes = list(range(1, trainer.num_classes)) # 1
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    :param list_of_lists: [[case0_0000.nii, case0_0001.nii], [case1_0000.nii, case1_0001.nii], ...]
    :param output_filenames: [output_file_case0.nii, output_file_case1.nii, ...]
    ...
    for o in output_filenames:
        ...
        if not f.endswith(".nii"):
            f, _ = os.path.splitext(f) # 分离文件名和扩展名
            f = f + ".nii"
    ...
    if not overwrite_existing:
        ...
        cleaned_output_files = [cleaned_output_files[i] for i in not_done_idx] #所有预测结果路径名
        list_of_lists = [list_of_lists[i] for i in not_done_idx]  #所有测试图像路径名
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    ...
    files = subfiles(input_folder, suffix=".nii", join=False, sort=True) # 拿到测试图像列表
    ...
    assert len(files) > 0, "input folder did not contain any images (expected to find .nii file endings)"

    # now check if all required files are present and that no unexpected files are remaining
    for c in maybe_case_ids:
        for n in range(expected_num_modalities):
            expected_output_file = c + "_%04.0d.nii" % n
    ...
    print("If they don't look right, make sure to double check your filenames. They must end with _0000.nii etc")
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    case_ids = check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities) # 拿到所有测试图像文件名

    output_files = [join(output_folder, i + ".nii") for i in case_ids] #得到所有的结果文件名(包括扩展名)
    all_files = subfiles(input_folder, suffix=".nii", join=False, sort=True)  # 拿到所有测试图像
    list_of_lists = [[join(input_folder, i) for i in all_files if i[:len(j)].startswith(j) and
                      len(i) == (len(j) + 9)] for j in case_ids]  # 拿到所有测试图像路径

    if lowres_segmentations is not None:
        assert isdir(lowres_segmentations), "if lowres_segmentations is not None then it must point to a directory"
        lowres_segmentations = [join(lowres_segmentations, i + ".nii") for i in case_ids]
        assert all([isfile(i) for i in lowres_segmentations]), "not all lowres_segmentations files are present. " \
                                                               "(I was searching for case_id.nii in that folder)"
...
#                                                      "CASENAME_XXXX.nii where XXXX is the modality "
