...
from utilities.file_and_folder_operations import save_json, subfiles, join
...
class Evaluator:
    ...
    def __init__(self,
        ...
        if metrics is None:
            ...
            for m in metrics:
                self.metrics.append(m)
        self.metrics.append("Hausdorff Distance 95")
...
def evaluate_folder(folder_with_gts: str, folder_with_predictions: str, labels: tuple, **metric_kwargs):
    ...
    :return:
    """
    files_gt = subfiles(folder_with_gts, suffix=".nii", join=False)
    files_pred = subfiles(folder_with_predictions, suffix=".nii", join=False)
    ...
    test_ref_pairs = [(join(folder_with_predictions, i), join(folder_with_gts, i)) for i in files_pred] #列表存储所有预测label以及原label的路径
