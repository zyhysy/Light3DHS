...
def main():
    ...
    parser.add_argument("task", help="can be task name or task id") # 24
    parser.add_argument("fold", help='0, 1, ..., 5 or \'all\'') # 3
    parser.add_argument("-val", "--validation_only", help="use this if you want to only run the validation",
                        action="store_true") # val默认值为false，设置action="store_true"则意味着只要运行时该变量有传参就将该变量设为True
    ...
    parser.add_argument("--deterministic",
                        help="Makes training deterministic, but reduces training speed substantially. I (Fabian) think "
                             ...
                             "Don't use that.",
                        required=False, default=False, action="store_true") # 使训练具有确定性，但这是不必要的。确定性训练会使你过度适应一些随机种子。
    parser.add_argument("--npz", required=False, default=False, action="store_true", help="if set then nnUNet will "
                                                                                          ...
                                                                                          "you should enable this") #如果设置，则nnUNet也将导出验证中预测部分的npz文件。
    parser.add_argument("--find_lr", required=False, default=False, action="store_true",
                        help="not used here, just for fun")
    parser.add_argument("--valbest", required=False, default=False, action="store_true",
                        help="hands off. This is not intended to be used")
    parser.add_argument("--fp32", required=False, default=False, action="store_true",
                        help="disable mixed precision training and run old school fp32") #用来禁用混合精度训练病运行老的fp32
    ...
    if not task.startswith("Task"):
        task_id = int(task)
        task = convert_id_to_task_name(task_id) # Task024_HarpL
    ...
    trainer_class = get_default_configuration(network, task, network_trainer, plans_identifier) # 获取默认配置
    #print(trainer_class) # training.network_training.nnUNetTrainerV2.nnUNetTrainerV2
    ...
    ##print(trainer) # <training.network_training.nnUNetTrainerV2.nnUNetTrainerV2 object at 0x7f4015b56040>
    ...
    trainer.initialize(not validation_only)  # 初始化训练器
